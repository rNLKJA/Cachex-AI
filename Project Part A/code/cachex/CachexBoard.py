...
class CachexBoard:
    ...
    def AStar(self, start=None, goal=None, heuristic="manhattan", p=None, block=None):
        ...
        AStarScores[start].h = distance_diff(
            point1=start, point2=goal, heuristic=heuristic, p=p
        )
                ...
                if block is not None:
                    if self.NodeDict[nextNode].state != block:
                        ...
                        if AStarScores[currentNode].g + 1 < AStarScores[nextNode].g:
                            ...
                            AStarScores[nextNode].h = distance_diff(
                                point1=nextNode, point2=goal, heuristic=heuristic, p=p
                            )
                            ...
                            if nextNode not in queueTracker:
                                insert_order += 1
                                priorityQueue.put(
                                    [AStarScores[nextNode].f, insert_order, nextNode]
                                )
