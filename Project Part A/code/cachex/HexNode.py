...
class HexNode:
    ...
    def distance_diff(
        self, point1: tuple, point2: tuple, heuristic="manhattan", p=None
    ):
        """
    ...
    def minkowski(self, point1: tuple, point2: tuple, p: int):
        ...
        """
        return pow(
            pow(abs(point1[0] - point2[0]), p) + pow(abs(point1[1] - point2[1]), p),
            1 / p,
        )
    ...
    def find_next_moves(self, board: CachexBoard, inplace=True, verbose=0):
        ...
        for r in range(self.coord[0] - 1, self.coord[0] + 2):
            for q in range(self.coord[1] - 1, self.coord[1] + 2):
                if (r, q) in board:
                    if verbose == 1:
                        print(
                            f"Expanding node: {self.coord}, generated next valid move {(r, q)}"
                        )
        ...
        if verbose == 1:
            print(
                f"Removing diagonal element {(self.coord[0]-1, self.coord[1]-1)}, {(self.coord[0]+1, self.coord[1]+1)}"
            )
