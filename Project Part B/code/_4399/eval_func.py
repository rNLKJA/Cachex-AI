...
from numpy import array, roll, vectorize, zeros
...
_TRIANGLE_PATTERNS = [[n1, n2] for n1, n2 in list(zip(_HEX_STEPS, roll(_HEX_STEPS, 1)))]

_DIAMOND_PATTERNS = [
    [_ADD(n1, n2), n1, n2]
    for n1, n2 in list(zip(_HEX_STEPS, roll(_HEX_STEPS, 1)))
    + list(zip(_HEX_STEPS, roll(_HEX_STEPS, 2)))
]

_WEAK_PATTERNS1 = [[n1, _ADD(n1, n1)] for n1 in _HEX_STEPS]
...
def estimate_steps_to_win(board):
    ...
    for reachable in connected_nodes:
        ...
        max_token = [
            coord for coord in reachable if coord[_PLAYER_AXIS[player]] == max_score
        ][0]
        min_token = [
            coord for coord in reachable if coord[_PLAYER_AXIS[player]] == min_score
        ][0]
        ...
        if _PLAYER_AXIS[player]:
            ...
            sub_steps1 = AStar(
                Board=board,
                start=max_token,
                goal=(max_token[0], board.n - 1),
                self_player=player_num,
            )
            sub_steps2 = AStar(
                Board=board,
                start=min_token,
...
def score_matrix(board_size: int):
    ...
    for n in range(1, board_size // 2):
        tmp_matrix = (
            np.zeros((board_size - n - margin, board_size - n - margin))
            + int((board_size + 3) / 2)
            - n
        )
